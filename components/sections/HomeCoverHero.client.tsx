...
export function HomeCoverHero({
  ...
  const { currentSlide, setCurrentSlide, handlers } = useCarousel(COVER_HERO_SLIDE_IMAGES.length, 5000);
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className="relative z-10 flex flex-col items-center justify-center px-6 py-16 sm:min-h-[100svh] sm:px-10 sm:py-20 lg:px-12 lg:py-24">
        <div className="mx-auto w-full max-w-7xl">
          <div className="grid items-center gap-6 sm:gap-12 lg:grid-cols-2 lg:gap-20">
            
            {/* Left: Text Content */}
            <div className={`relative z-10 mx-auto max-w-2xl text-center sm:mx-0 sm:text-left transition-all duration-1000 motion-reduce:transition-none motion-reduce:opacity-100 motion-reduce:translate-y-0 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
              {/* Logo */}
              ...
              <div 
                className={`mb-4 hidden items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 backdrop-blur-sm sm:mb-6 sm:inline-flex sm:px-4 sm:py-2 transition-all duration-1000 motion-reduce:transition-none motion-reduce:opacity-100 motion-reduce:translate-y-0 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
                style={{ transitionDelay: "200ms" }}
              >
                <span className="relative flex h-1.5 w-1.5 sm:h-2 sm:w-2">
                  <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-green-400 opacity-75 motion-reduce:hidden"></span>
              ...
              <h1
                id="home-cover-title"
                className={`mb-3 text-2xl font-black uppercase tracking-tight text-white sm:mb-6 sm:text-3xl md:text-4xl lg:text-5xl transition-all duration-1000 motion-reduce:transition-none motion-reduce:opacity-100 motion-reduce:translate-y-0 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              ...
              {/* Description */}
              <p 
                className={`mx-auto mb-4 max-w-lg text-sm leading-relaxed text-white/80 sm:mx-0 sm:mb-10 sm:text-lg md:text-xl lg:text-2xl transition-all duration-1000 motion-reduce:transition-none motion-reduce:opacity-100 motion-reduce:translate-y-0 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              ...
              {/* CTAs */}
              <div 
                className={`flex flex-row items-center justify-center gap-2 sm:justify-start sm:gap-4 transition-all duration-1000 motion-reduce:transition-none motion-reduce:opacity-100 motion-reduce:translate-y-0 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
            ...
            <div 
              className={`relative hidden lg:block transition-all duration-1000 motion-reduce:transition-none motion-reduce:opacity-100 motion-reduce:scale-100 ${isLoaded ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
              style={{ transitionDelay: "300ms" }}
            >
              <div className="relative">
                ...
                <div className="absolute -bottom-6 -right-6 h-32 w-32 rounded-full bg-primary/30 blur-2xl motion-reduce:hidden" />
              </div>
            </div>
          </div>
        </div>
...
// Hook to manage carousel state
function useCarousel(length: number, intervalTime: number) {
  ...
  }, [length, intervalTime, isPaused]);

  // Handlers to pause carousel on hover/focus
  const handlers = {
  ...
  return { currentSlide, setCurrentSlide, handlers };
}
