...
export function HomeStoryStack({ items }: HomeStoryStackProps) {
  return (
    <section aria-label="About and agenda" className="relative overflow-hidden bg-neutral-ink py-16 sm:py-20 md:py-28 lg:py-32">
          ...
          {items.map((item, idx) => {
            ...
            return (
              <article
                key={item.title}
                className={[
                  "group relative grid items-center gap-5 overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm transition-all duration-500 hover:border-white/20 hover:bg-white/[0.08]",
